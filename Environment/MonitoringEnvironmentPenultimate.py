...
class MultiAgentMonitoring:
	...
	def __init__(self, 
		...
		self.scaled_std_sensormeasure = self.scaled_std_sensormeasure = np.abs((self.std_sensormeasure - self.range_std_sensormeasure[0]) * self.scale_std - 1) # used to inform network the std of every agent though the state (0.25 worst, 1 best)
		self.variance_sensormeasure = self.std_sensormeasure**2 # variance = std^2
		self.normalized_variance_sensormeasure =  (self.variance_sensormeasure - self.range_std_sensormeasure[0]**2) / (self.range_std_sensormeasure[1]**2 - self.range_std_sensormeasure[0]**2 ) # used in DQN as input for network_with_sensornoises to normalize variance between diferent agents
...
if __name__ == '__main__':
	...
	# std_sensormeasure = np.array([0.1, 0.0071, 0.0006, 0.0004])[:n_agents] # std of the measure of every agent
