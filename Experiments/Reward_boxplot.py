...
# Read the data
folder = 'Experiments/Results/COMPARATIVA GENERAL ENTRE ALGORITMOS'
...
errors = np.array([df.groupby('Run')['R_acc'].last() for df in dfs])
...
plt.ylabel('Accumulated Reward', fontsize=20)
plt.xlabel('')
# plt.title('Boxplots of the MSE for each algorithm and agents combination')
plt.tight_layout()
plt.savefig(fname=f"{folder}/Reward_boxplot.svg")
plt.show()
